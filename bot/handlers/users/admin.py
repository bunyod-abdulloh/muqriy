...
@router.message(Command('allusers'), IsBotAdminFilter(ADMIN_ID))
...
@router.message(Command('reklama'), IsBotAdminFilter(ADMIN_ID))
async def ask_ad_content(message: types.Message, state: FSMContext):
    await message.answer("Reklama uchun post yuboring")
    await state.set_state(AdminState.ask_ad_content)


@router.message(AdminState.ask_ad_content, IsBotAdminFilter(ADMIN_ID))
...
@router.message(Command('cleandb'), IsBotAdminFilter(ADMIN_ID))
...
@router.callback_query(AdminState.are_you_sure, IsBotAdminFilter(ADMIN_ID))
